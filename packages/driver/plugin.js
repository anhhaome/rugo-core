...
export default {
  ...
  async start(context) {
    ...
    for (let key in context.drivers)
      console.log(colors.yellow('[driver] ') + colors.white(`"${key}" driver is `) + (context.drivers[key] ? colors.green(`ready`) : colors.red(`not ready`)));
  },
  async close(context) {
    for (let driverName in context.drivers)
      if (context.drivers[driverName])
        await context.drivers[driverName].close();
  }
}